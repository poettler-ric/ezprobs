...
bp = Blueprint("free_surface_fit_fit_33", __name__)
...
def plot_function():
    ...
    if ks_1 == ks_2:
        ...
        x_pre = []
        x_pre.append(x_start)
        x_pre.append(x_a - l_au)
        tr.append(t_n1)
        tr.append(t_n1)

        ys = np.linspace(t_n1, t_crit, 22)
        for y in ys[1:-1]:
            x_pre.append(x_a - ruehlmann_rect(y, t_n1, t_crit, t_crit, i))
            tr.append(y)
        x_pre.append(x_a)
        tr.append(t_crit)

        x_pre = np.array(x_pre)
        y_pre = (i * (x_max - x_pre)) + np.array(tr)

        # quadratic decline after a
        ...
        positions = np.linspace(0, min(l_ad ,x_max), 100)
